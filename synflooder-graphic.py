...
target_ip_entry = tk.Entry(root, width=entry_width, fg=entry_fg, bg=entry_bg)
...
local_ip_entry = tk.Entry(root, width=entry_width, fg=entry_fg, bg=entry_bg)
...
port_entry = tk.Entry(root, width=entry_width, fg=entry_fg, bg=entry_bg)
...
source_port_entry = tk.Entry(root, width=entry_width, fg=entry_fg, bg=entry_bg)
source_port_entry.grid(row=3, column=1, sticky="w")
source_port_entry.insert(0, "12228")

tk.Label(root, text="发送速度:", font=label_font, fg=label_fg, bg=label_bg, anchor="w").grid(row=4, column=0, sticky="w")
...
speed_radio_1.grid(row=4, column=1, sticky="w")
speed_entry = tk.Entry(root, width=entry_width - 10, fg=entry_fg, bg=entry_bg)
speed_entry.grid(row=4, column=2, sticky="w")
speed_entry.insert(0, "1000")

speed_radio_2 = tk.Radiobutton(root, text="超负荷泛洪", variable=speed_var, value=2, command=toggle_speed_entry, font=label_font, fg=label_fg, bg=label_bg, selectcolor='black')
speed_radio_2.grid(row=5, column=1, sticky="w")

syn_button = tk.Button(root, text="开始 SYN 攻击", command=start_syn_attack, font=button_font, fg=label_fg, bg=label_bg, width=15)
syn_button.grid(row=6, column=0, pady=5, sticky="w")

udp_button = tk.Button(root, text="开始 UDP 攻击", command=start_udp_attack, font=button_font, fg=label_fg, bg=label_bg, width=15)
udp_button.grid(row=6, column=1, pady=5, sticky="w")

stop_button = tk.Button(root, text="停止攻击", command=stop_attack, font=button_font, fg=label_fg, bg=label_bg, width=15)
stop_button.grid(row=7, columnspan=2, pady=5)
...
project_link.grid(row=8, columnspan=2, sticky="w")
project_link.bind("<Button-1>", open_project_link)

author_name = tk.Label(root, text="作者：GR3YPH_4NTOM | 版本：v1.1", font=label_font, fg=label_fg, bg=label_bg, anchor="w")
author_name.grid(row=9, columnspan=2, sticky="w")
